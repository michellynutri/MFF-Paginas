    ...
    <div className="sticky top-0 z-50 bg-verde-esc px-4 py-3">
      <div className="mx-auto flex max-w-4xl flex-wrap items-center justify-center gap-2 text-center">
        <span className="font-sans text-sm font-medium text-white">
          Aula 03 no ar agora. Sai do ar em:
        </span>
...
function VideoThumbnail() {
    ...
    return (
      <div className="relative overflow-hidden rounded-2xl">
        <div className="aspect-video w-full">
          <iframe
            src="https://www.youtube.com/embed/c-eyuRW6YKE?autoplay=1&rel=0"
            title="Aula 03 - As 3 decisões que determinam se você vai se orgulhar do resultado"
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl">
      {/* Thumbnail da Aula 03 */}
      <div className="aspect-video w-full">
        <img
          src="/images/thumb-aula-3.png"
          alt="Aula 03 - As 3 decisões que determinam se você vai se orgulhar do resultado"
          className="h-full w-full object-cover"
        />
        {/* Play button overlay */}
        <button 
...
function LessonCard({ lessonNumber, title, date, time, description, isActive, isLocked, isWatched, thumbnail, videoUrl }: LessonCardProps) {
  const CardWrapper = videoUrl && !isLocked ? "a" : "div"
  const cardProps = videoUrl && !isLocked ? { href: videoUrl, target: "_blank", rel: "noopener noreferrer" } : {}
  
  return (
    <CardWrapper {...cardProps} className={`block overflow-hidden rounded-2xl bg-branco shadow-card transition-all ${isLocked ? "" : "hover:-translate-y-1 cursor-pointer"}`}>
      {/* Thumbnail */}
      ...
      <div className="p-5">
        <div className={`mb-3 h-1 w-12 rounded-full ${isActive ? "bg-verde-vita" : isWatched ? "bg-verde-vita/50" : "bg-borda"}`} />
        ...
        <p className={`mt-1 font-sans text-xs ${isActive ? "font-medium text-verde-vita" : isWatched ? "text-verde-vita/70" : "text-texto/50"}`}>
          {isActive ? "Assistindo" : isWatched ? "Assistido" : `${date}`}
        </p>
...
export default function CPL3Page() {
  // Data de expiração da Aula 03 (25 de março às 18h)
  const aula03Expiration = new Date("2026-03-25T18:00:00")
  // Data do evento (26 de março às 20h)
  const eventoDate = new Date("2026-03-26T20:00:00")

  return (
    <main className="min-h-screen bg-creme">
      {/* ── BLOCO 1: BARRA STICKY ─────────────────────────────────────────── */}
      <StickyBar targetDate={aula03Expiration} />
      ...
      <section className="bg-creme px-5 py-8 lg:py-12">
        <div className="mx-auto max-w-4xl">
          ...
          {/* Avatares de Aula (linha horizontal) - Aula 03 ativa */}
          <div className="mb-10 flex items-center justify-center gap-4 overflow-x-auto pb-2 md:gap-6">
            <LessonAvatar number="01" date="Assistido" avatar="/images/avatar-aula-1.png" />
            <LessonAvatar number="02" date="Assistido" avatar="/images/avatar-aula-2.png" />
            <LessonAvatar number="03" date="Assistindo" isActive avatar="/images/avatar-aula-3.png" />
            <LessonAvatar number="04" date="26/03" label="Evento" avatar="/images/avatar-evento.png" />
          </div>

          {/* Card da Aula Ativa - Aula 03 */}
          <div className="mx-auto max-w-2xl">
            <VideoThumbnail />

            {/* Descrição da Aula 03 */}
            <div className="mt-6 rounded-2xl bg-branco p-6 shadow-card">
              <p className="mb-1 font-sans text-sm font-bold text-verde-esc">AULA 03</p>
              <h2 className="font-serif text-xl font-bold text-marrom md:text-2xl">
                As 3 decisões que determinam se você vai se orgulhar do resultado — ou recomeçar do zero.
              </h2>
              <p className="mt-4 font-sans text-base leading-relaxed text-texto/80">
                Os estudos mostram que até 70% das mulheres que usam GLP-1 recuperam peso significativo nos primeiros 12 meses após parar. Não porque a caneta não funcionou. Porque o metabolismo não foi treinado para seguir sozinho. Nessa aula eu te mostro as três decisões que separam essas mulheres: se você está usando a caneta como ferramenta ou como muleta, se a sua suplementação está resolvendo sintoma ou construindo resultado, e se você já está preparando o desmame.
              </p>
              ...
              <div className="mt-8 rounded-xl bg-creme p-6">
                ...
                <CountdownTimer targetDate={eventoDate} />
              </div>

              {/* CTA WhatsApp */}
              <div className="mt-6">
                <p className="mb-4 text-center font-sans text-sm text-texto/70">
                  Entre no grupo do WhatsApp para receber o aviso quando o evento começar
                </p>
                <a
...
{/* Aula 01 - Já assistida */}
  ...
  videoUrl="https://youtu.be/iw6CuewAA-8"
  isWatched
  />

{/* Aula 02 - Já assistida */}
  <LessonCard
  lessonNumber="02"
  ...
  videoUrl="https://youtu.be/p9UxGnxa5uw"
  isWatched
  />
            ...
            <LessonCard
              ...
              date="24/03"
              time="18h"
              description={["Os estudos mostram que até 70% das mulheres que usam GLP-1 recuperam peso significativo nos primeiros 12 meses após parar. Nessa aula eu te mostro as três decisões que separam essas mulheres."]}
